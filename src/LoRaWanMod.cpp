// based on LoRaWanMinimal_APP.cpp from HELTEC
#include "LoRaWanMod.h"
// #include <LoRaWanMod_APP.h>

#if (AT_SUPPORT)
#error                                                                         \
    "The LoRaWan library is configured for LORAWAN_AT_SUPPORT=OFF. Please disable."
#endif
...
#elif defined(REGION_AS923) || defined(REGION_AS923_AS1) ||                    \
    defined(REGION_AS923_AS2)
...
void LoRaWanMod::begin(DeviceClass_t loraWanClass, LoRaMacRegion_t region) {
  ...
  if (itsRegion == LORAMAC_REGION_AS923_AS1 ||
      itsRegion == LORAMAC_REGION_AS923_AS2)
...
bool LoRaWanMod::joinOTAA(uint8_t *appEui, uint8_t *appKey) {
  return joinOTAA(appEui, appKey, generateDevEUIByChipID());
}

bool LoRaWanMod::joinOTAA(uint8_t *appEui, uint8_t *appKey, uint8_t *devEui) {
  MlmeReq_t mlmeReq;
...
bool LoRaWanMod::joinABP(uint8_t *nwkSKey, uint8_t *appSKey, uint32_t devAddr) {
  MibRequestConfirm_t mibReq;
...
bool LoRaWanMod::send(uint8_t datalen, uint8_t *data, uint8_t port,
                      bool confirmed) {
  MibRequestConfirm_t mibReq;
  mibReq.Type = MIB_DEVICE_CLASS;
  LoRaMacMibGetRequestConfirm(&mibReq);
  ...
  if (LoRaMacQueryTxPossible(datalen, &txInfo) != LORAMAC_STATUS_OK) {
    ...
    if (confirmed) {
      ...
      mcpsReq.Req.Confirmed.NbTrials = this->itsNumRetries;
  ...
  if (status != LORAMAC_STATUS_OK) {
    // printf("LoRaWanMod_APP.send: Bad LoRaMacMcpsRequest status, couldn't
    // "
    //        "send %d",
    //        status);
    return false;
...
static void MlmeConfirm(MlmeConfirm_t *mlmeConfirm) {
  ...
  case MLME_JOIN: {
    if (mlmeConfirm->Status == LORAMAC_EVENT_INFO_STATUS_OK) {
      ...
      //       turnOffRGB();
      // #endif
      LoRaWanModified.setJoined(true);
    } else {
      LoRaWanModified.setJoined(false);
    }
    break;
  }
...
static void MlmeIndication(MlmeIndication_t *mlmeIndication) {
  ...
  case MLME_SCHEDULE_UPLINK: { // The MAC signals that we shall provide an
                               // uplink as soon as possible
...
uint8_t *LoRaWanMod::generateDevEUIByChipID() {
